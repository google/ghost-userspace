...
def GetDefaultRocksDBWorkerCpus():
    ...
    return list(
        range(
            _FIRST_ROCKSDB_WORKER_CPU, _FIRST_ROCKSDB_WORKER_CPU + _NUM_ROCKSDB_WORKERS
        )
    )
...
def GetRocksDBOptions(scheduler: Scheduler, num_cpus: int, num_workers: int):
    ...
    # for workers.
    if not (
        (scheduler == Scheduler.CFS and num_cpus >= 3)
        or (scheduler == Scheduler.GHOST and num_cpus >= 2)
    ):
        raise ValueError(f"Incorrect number of CPUs specified for {scheduler.value}.")
    if num_workers < 1:

...
#ifndef GHOST_EXPERIMENTS_SYNTHETIC_GHOST_ORCHESTRATOR_H_
#define GHOST_EXPERIMENTS_SYNTHETIC_GHOST_ORCHESTRATOR_H_
...
private:
    ...
    // Initializes the ghOSt PrioTable. Note that this should only be called
    // when the ghOSt wait type is `Orchestrator::GhostWaitType::kPrioTable`.
    void InitPrioTable();

    // Returns true if workers wait on a PrioTable.
    bool UsesPrioTable() const {
        return options().ghost_wait_type == GhostWaitType::kPrioTable;
    }
    // Returns true if workers wait on futexes.
    ...
    // runtimes, different QoS (Quality-of-Service) classes, etc.) across
    // workers in our experiments. Furthermore, all workers are ghOSt one-shots
    // and the only candidate for a repeatable -- the load generator -- runs in
    // CFS. Thus, put all worker sched items in the same work class.
    ...
    // until they are marked runnable again. Note this is only used when the
    // ghOSt wait type is `Orchestrator::GhostWaitType::kFutex`. Otherwise, the
    // pointer is null.
    ...
    // `Orchestrator::GhostWaitType::kPrioTable`. Otherwise, the pointer is
    // null.
    ...
    // member rather than a local variable in the 'LoadGenerator' method to
    // avoid repeatedly allocating memory for the list backing in the load
    // generators' common case, which is expensive.
    std::vector<std::vector<uint32_t>> idle_sids_;
};

} // namespace ghost_test

#endif // GHOST_EXPERIMENTS_SYNTHETIC_GHOST_ORCHESTRATOR_H_

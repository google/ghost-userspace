...
#ifndef GHOST_EXPERIMENTS_SYNTHETIC_INGRESS_H_
#define GHOST_EXPERIMENTS_SYNTHETIC_INGRESS_H_

#include "absl/random/bit_gen_ref.h"
#include "absl/time/clock.h"
#include "experiments/synthetic/clock.h"
...
class Ingress {
public:
    // Constructs the ingress queue. `throughput` is the generated throughput
    // (so `throughput` is used as the lambda for the Possion arrival process).
    ...
    // Models a Poisson arrival process with a lambda of `throughput_`. Returns
    // a pair with 'true' and the arrival time when at least one request is
    // waiting in the ingress queue. Returns a pair with 'false' and an
    // undefined arrival time when no request is waiting in the ingress queue.
...
private:
    ...
    absl::Duration NextDuration() {
        // To help avoid issues due to double precision, we convert
        // `throughput_` from units of 'requests per second' to 'requests per
        // millisecond'.
        double duration_msec = absl::Exponential(gen_, throughput_ / 1000.0);
        return absl::Milliseconds(duration_msec);
    }

    // The target throughput for the ingress queue. This throughput is used as
    // the lambda for the Poisson arrival process.
    const double throughput_;
    // The clock. This can either be a 'RealClock' or a 'SimulatedClock'
    // (which generally only tests will want so they can test deterministic
    // behavior).
    Clock &clock_;
    // The time that the ingress queue started at.
    absl::Time start_ = absl::UnixEpoch();
    // 'absl::BitGen' is not thread safe, but each instance of this class will
...
public:
    ...
    // throughput of `throughput` and is backed by a Poisson arrival process.
    // The fraction of requests that are Range queries is `range_query_ratio`
    // and the fraction of requests that are Get requests is 1 -
    // `range_query_ratio_`. Note that `range_query_ratio` must be greater than
    // or equal to 0.0 and less than or equal to 1.0. `clock` is the clock to
    // use and is a `RealClock` by default. A `SimulatedClock` could be passed
    // instead though generally only unit tests will want to do this so they can
    // test deterministic behavior.
    SyntheticNetwork(double throughput, double range_query_ratio,
                     Clock &clock = GetRealClock());

    // Starts the synthetic network. No requests are synthetically generated
    // until this method is called.
    void Start();
    // Polls the synthetic ingress queue. Returns true and fills in `request`
    // with the request at the front of the queue, if one exists. Returns false
    // if there is no request in the queue; the value at the memory location
    // pointed to by `request` is undefined in this case.
...
private:
    ...
    Ingress ingress_;
    // The fraction of requests that are Range queries is `range_query_ratio_`
    // and the fraction of requests that are Get requests is 1 -
    // `range_query_ratio_`. Note that `range_query_ratio_` is greater than or
...
#endif // GHOST_EXPERIMENTS_SYNTHETIC_INGRESS_H_

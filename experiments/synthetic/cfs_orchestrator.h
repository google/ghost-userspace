...
#ifndef GHOST_EXPERIMENTS_SYNTHETIC_CFS_ORCHESTRATOR_H_
#define GHOST_EXPERIMENTS_SYNTHETIC_CFS_ORCHESTRATOR_H_
...
private:
    ...
    // The dispatcher calls this method to receive requests sent to it by the
    // load generator.
    void HandleLoadGenerator(uint32_t sid);

    // The dispatcher calls this method to populate 'idle_sids_' with a list of
    // the SIDs of idle workers. Note that this method clears 'idle_sids_'
    ...
    // decrements this once they have initialized themselves. This barrier is
    // used to block the load generator until all threads have been initialized
    // so that it does not generate load while the system is initializing. If it
    // generated load while the system is initializing, the experiment results
    // would be bad solely due to initialization costs rather than any
    // deficiency in the system. The initialization costs are irrelevant to the
    // experiment.
    ...
    // rather than a local variable in the 'Dispatcher' method to avoid
    // repeatedly allocating memory for the list backing in the dispatchers'
...
#endif // GHOST_EXPERIMENTS_SYNTHETIC_CFS_ORCHESTRATOR_H_

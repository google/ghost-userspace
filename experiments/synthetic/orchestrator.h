...
#ifndef GHOST_EXPERIMENTS_SYNTHETIC_ORCHESTRATOR_H_
#define GHOST_EXPERIMENTS_SYNTHETIC_ORCHESTRATOR_H_

#include <filesystem>

#include "experiments/shared/thread_pool.h"
#include "experiments/shared/thread_wait.h"
#include "experiments/synthetic/ingress.h"
#include "experiments/synthetic/latency.h"
#include "experiments/synthetic/request.h"
...
struct Options {
    ...
    ghost::CpuList load_generator_cpus =
        ghost::MachineTopology()->EmptyCpuList();

    // For CFS (Linux Completely Fair Scheduler) experiments, the CPUs that the
    // dispatchers run on.
    ghost::CpuList cfs_dispatcher_cpus =
        ghost::MachineTopology()->EmptyCpuList();
    ...
    // The total amount of time spent processing a Get request and doing
    // synthetic work.
    absl::Duration get_duration;

    // The total amount of time spent processing a Range query and doing
    // synthetic work.
...
// This class is the central orchestrator of the synthetic experiment. It
// manages the load generator, the dispatcher (if one exists), and the workers.
// It prints out the results when the experiment is finished.
//
...
public:
    ...
    // requests to workers. In the ghOSt experiments, the load generator uses
    // this to pass requests to workers.
    //
    // When 'num_requests' is greater than zero, there are pending requests for
    // the worker. When 'num_requests' is 0, there are no pending requests for
    // the worker, so the dispatcher should add requests.
    struct WorkerWork {
        // The number of requests in 'requests'. We use this atomic rather than
        // just look at 'requests.size()' since the dispatcher and the worker
        // need an atomic to sync on. This number should never be greater than
        // 'options_.batch'.
...
protected:
    ...
    // 'total_threads' is the total number of threads managed by the
    // orchestrator, including the load generator thread, the worker threads,
    // and if relevant, the dispatcher thread.
    Orchestrator(Options options, size_t total_threads);

    // This method is executed in a loop by the load generator thread. This
    // method checks the ingress queue for pending requests. 'sid' is the sched
    // item identifier for the load generator thread.
    virtual void LoadGenerator(uint32_t sid) = 0;

    // This method is executed in a loop by the dispatcher thread, if one
    // exists. If so, this method receives requests from the load generator and
    // assigns them to workers. 'sid' is the sched item identifier for the
    // dispatcher thread.
    virtual void Dispatcher(uint32_t sid) = 0;

    // This method is executed in a loop by worker threads. This method handles
    // the request assigned to it by doing synthetic work. 'sid' is the sched
    // item identifier for the worker thread.
    virtual void Worker(uint32_t sid) = 0;

    // Handles 'request' which is either a Get request or a Range query. 'gen'
    // is a random bit generator used for Get requests that have an exponential
...
private:
    ...
    // not).
    void HandleRange(Request &request, std::string &response,
                     absl::BitGen &gen);

    // Prints the results for 'requests' (total number of requests in
    // 'requests', throughput, and latency percentiles). 'results_name' is a
    // name printed with the results (e.g., "Get" for Get requests) and
    // 'experiment_duration' is the duration of the experiment.
    ...
    std::vector<Request>
    FilterRequests(const std::vector<std::vector<Request>> &requests,
    ...
    // thread when calling this method. There is overhead to calling this
    // method, such as creating the stack frame, so passing 'start_duration'
    // allows the method to count this overhead toward its synthetic work.
    void Spin(absl::Duration duration, absl::Duration start_duration) const;

    // Orchestrator options.
    const Options options_;

    // The total number of threads managed by the orchestrator, including the
    // load generator thread, the worker threads, and if relevant, the
    // dispatcher thread.
    ...
    // 'i' accesses index 'i' in this vector. We wrap each 'WorkerWork' struct
    // in a unique pointer since the struct contains an atomic and therefore
    // does not have a copy constructor, so it cannot be stored directly into a
    // vector.
    ...
    // Random bit generators. Each thread has its own bit generator since the
    // bit generators are not thread safe.
...
#endif // GHOST_EXPERIMENTS_SYNTHETIC_ORCHESTRATOR_H_

...
#include "experiments/synthetic/cfs_orchestrator.h"
...
void CfsOrchestrator::Terminate() {
    ...
    // generator logic changes in the future to always expect the dispatcher to
    // be alive while it is running.
    ...
    while (thread_pool().NumExited() < total_threads()) {
        for (size_t i = 0; i < options().num_workers; ++i) {
            thread_wait_.MarkRunnable(i + num_load_generators +
                                      num_dispatchers);
        }
    }
    thread_pool().Join();
...
void CfsOrchestrator::LoadGenerator(uint32_t sid) {
    ...
    if (!first_run().Triggered(sid)) {
        ...
        CHECK_EQ(ghost::GhostHelper()->SchedSetAffinity(
                 ...
                 0);
        // Use 'printf' instead of 'std::cout' so that the print contents do not
        // get interleaved with the dispatcher's, the workers', and the main
        // thread's print contents. 'printf' acquires a lock whereas 'std::cout'
        // does not.
        ...
        // before starting the timer and generating load. If we started
        // generating load before the dispatcher and workers are initialized, we
        // will not have functional correctness issues but the load will not be
        // processed, causing us to report bad performance at the end of the
        // experiment that solely reflects initialization costs, which are
    ...
    for (size_t i = 0; i < num_dispatchers; i++) {
        ...
        for (size_t i = 0; i < kLoadGeneratorBatchSize; ++i) {
            ...
            if (network(sid).Poll(request)) {
                ...
                // requests we have so far to the dispatcher.
                break;
            }
        }
...
void CfsOrchestrator::HandleLoadGenerator(uint32_t sid) {
    ...
    if (load_count > 0) {
        ...
        // The dispatcher is not writing anything visible to the load generator
        // in this critical section, so write to 'num_requests' with a relaxed
...
void CfsOrchestrator::GetIdleWorkerSIDs(uint32_t sid) {
    ...
    for (size_t i = sid - num_load_generators; i < options().num_workers;
         i += num_dispatchers) {
        // Skip the load generator and dispatcher threads, which have SIDs 0
        // through `num_load_generators + num_dispatchers - 1`.
...
void CfsOrchestrator::Dispatcher(uint32_t sid) {
    if (!first_run().Triggered(sid)) {
        ...
        printf("Dispatcher (SID %u, TID: %ld, affined to CPU %u)\n", sid,
               syscall(SYS_gettid), sched_getcpu());
        // The load generator will not start generating requests until itself,
        // the dispatcher, and all of the workers have initialized themselves,
        // so the dispatcher will never attempt to assign requests to
        // uninitialized workers because the load generator gives it no requests
        // to assign.
        threads_ready_.Block();
    }
    ...
    for (uint32_t i = 0; i < size; ++i) {
        ...
        CHECK_EQ(worker_work()[worker_sid]->num_requests.load(
                     std::memory_order_relaxed),
                 0);
        worker_work()[worker_sid]->requests.clear();

        for (uint32_t i = 0; i < options().batch; ++i) {
            // We need to check this here (in addition to above) since the
            // dispatcher queue may become empty before we assign
            // 'options.batch' requests to this worker.
            if (dispatcher_queue_[sid].empty()) {
                break;
            }
        ...
        if (!worker_work()[worker_sid]->requests.empty()) {
            ...
            CHECK_LE(worker_work()[worker_sid]->requests.size(),
                     options().batch);
...
void CfsOrchestrator::Worker(uint32_t sid) {
    if (!first_run().Triggered(sid)) {
        CHECK(first_run().Trigger(sid));
        // The first SIDs are given to the load generator and the dispatchers,
        // so subtract the load generator and the number of dispatchers to get
        // the worker's CPU assignment.
        CHECK_EQ(ghost::GhostHelper()->SchedSetAffinity(
                     ghost::Gtid::Current(),
                     ghost::MachineTopology()->ToCpuList(
        ...
        // Do this after 'MarkIdle'. If the worker did it before calling
        // 'MarkIdle', the dispatcher could assign work to this worker and then
        // mark it runnable. Then the worker could mark itself idle and go
        // spin/sleep on 'WaitUntilRunnable', causing the worker to do no work
        // for the duration of the experiment. Remember that 'MarkIdle' does not
        // make the worker spin/sleep -- only 'WaitUntilRunnable' does.
        threads_ready_.Block();
        thread_wait_.WaitUntilRunnable(sid);

        // Return here since it is possible the worker was never assigned a
        // request and is being woken up because the application is exiting.
        return;
    }
    ...
    thread_wait_.MarkIdle(sid);
    // Set 'num_requests' to 0 after calling 'thread_wait_.MarkIdle' to avoid
    // the same race mentioned above when the worker is initializing.

...
// Runs the synthetic test for ghOSt or CFS.
#include <csignal>

#include "absl/flags/parse.h"
#include "experiments/synthetic/cfs_orchestrator.h"
#include "experiments/synthetic/ghost_orchestrator.h"
...
ABSL_FLAG(absl::Duration, get_duration, absl::Microseconds(10),
          "The duration of Get requests.");
ABSL_FLAG(absl::Duration, range_duration, absl::Microseconds(10000),
          "The duration of Range queries.");
...
ghost_test::Options GetOptions() {
    ...
    options.print_options.distribution =
        absl::GetFlag(FLAGS_print_distribution);
...
int main(int argc, char *argv[]) {
    ...
    // 'ghost_test::Orchestrator::kBackgroundThreadCpu'. Also, since we affine
    // the main thread here before any background threads are spawned (e.g., the
    // synthetic worker background threads), the background threads will inherit
    // the main thread's affinity mask when they are spawned. Thus, the
    // background threads will automatically be affined to
    ...
    // `stdout` is directed to a non-interactive device (e.g, a Python
    // subprocess pipe), it is fully buffered. Thus, in order for the Python
    // script to read the initialization message as soon as it is passed to
    // `printf`, we need to manually flush `stdout`.
    fflush(stdout);

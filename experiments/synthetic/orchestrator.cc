...
#include "experiments/synthetic/orchestrator.h"
...
std::ostream &operator<<(std::ostream &os, const Options &options) {
    ...
    for (size_t i = 0; i < options.load_generator_cpus.Size(); i++) {
        load_generator_cpus +=
            options.load_generator_cpus.GetNthCpu(i).ToString();
    ...
    for (size_t i = 0; i < options.cfs_dispatcher_cpus.Size(); i++) {
        cfs_dispatcher_cpus +=
            options.cfs_dispatcher_cpus.GetNthCpu(i).ToString();
...
Orchestrator::Orchestrator(Options options, size_t total_threads)
    : options_(options), total_threads_(total_threads), gen_(total_threads),
      first_run_(total_threads), thread_pool_(total_threads) {
    ...
    for (size_t i = 0;
        ...
        // Reserve enough space in each worker's vector for at most 30 seconds
        // of requests (assuming the worker is handling all requests). We
        // reserve the memory upfront to avoid memory allocations during the
        // experiment. Memory allocations are expensive and could cause threads
        // to block on TCMalloc's mutex, which puts more work on the scheduler.
        const absl::Duration reserve_duration =
            std::min(options_.experiment_duration, absl::Seconds(30));
        // Add one second to 'reserve_duration' so that we do not run the risk
        // of doing a memory allocation just before the end of the experiment.
        const size_t reserve_size =
            absl::ToDoubleSeconds(reserve_duration + absl::Seconds(1)) *
            options_.throughput;
        requests_.back().reserve(reserve_size);

        // The first insert into a vector is slow, likely because the allocator
        // is lazy and needs to assign the physical pages on the first insert.
        // In other words, the virtual pages seem to be allocated but physical
        // pages are not assigned on the call to 'reserve'. We insert and remove
        // a few items here to handle the overhead now. Workers should not
        // handle this initialization overhead since that is not what the
...
void Orchestrator::HandleGet(Request &request, std::string &response,
    ...
    if (options_.get_exponential_mean > absl::ZeroDuration()) {
        service_time += Request::GetExponentialHandleTime(
            gen, options_.get_exponential_mean);
    }
...
void Orchestrator::Spin(absl::Duration duration,
    ...
    // only advances while the thread is running on a CPU, not while the thread
    // is preempted.
